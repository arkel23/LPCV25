...
def make_classid_classname(args):
    if args.dataset_name == 'cub':
        ...
        df = pd.read_csv(os.path.join(args.dataset_root_path, 'train_val.csv'))

        # file paths already include class name as in n02085620-Chihuahua/n02085620_5927.jpg so split str to get class name
        df[['class_name_full', 'file_id']] = df['dir'].str.split('/', 1, expand=True)
        df[['folder_id', 'class_name']] = df['class_name_full'].str.split('-', 1, expand=True)
    ...
    elif args.dataset_name in ('food', 'vegfru'):
        df = pd.read_csv(os.path.join(args.dataset_root_path, 'train_val.csv'))

        # file paths already include class name as in apple_pie/1005649.jpg so split str to get class name
        df[['class_name', 'file_id']] = df['dir'].str.split('/', 1, expand=True)
